...
# 53 "./mcc_generated_files/system/../capture/ccp1.h"
...
# 117 "./mcc_generated_files/system/../timer/tmr1.h"
void Timer1_StartSinglePulseAcquisition(void);
# 126 "./mcc_generated_files/system/../timer/tmr1.h"
...
# 161 "./mcc_generated_files/system/../timer/tmr1.h"
...
static uint16_t GetWaterTDS(uint32_t capturedFq, uint16_t temperature)
{
    ...
    conductivity = ((0.14F) * capturedFq)*0.8;
    conductivity = GetTempCompensatedConductivityValue(conductivity, temperature);

    waterTDS = (uint16_t) ((0.5f) * conductivity *0.35f);
...
void running_average_push(RunningAverage *ra, uint16_t value) {
    ...
    ra->index = (ra->index + 1) % 5;
    if (ra->count > 5) {
        ra->sum -= ra->values[(ra->index + 5 - 1) % 5];
        ra->count = 5;
    }
}
